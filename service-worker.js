...
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.map(k=>k!==CACHE&&k.startsWith('fluffy-')?caches.delete(k):null))))});
